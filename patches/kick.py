...
class KickVODIE(KickBaseIE):
    ...
    def _resolve_uuid_v7(self, channel, v7_id):
        # Kick migrated new VOD URLs to UUIDv7 (version nibble = '7' at position 14).
        # The v1/video/ backend only accepts UUIDv4. Resolve by matching the UUIDv7
        # timestamp against the channel's recent video listing (v2/channels/{ch}/videos).
        hex_ts = v7_id[:8] + v7_id[9:13]  # first 48 bits (ms timestamp) without hyphens
        ts_ms = int(hex_ts, 16)

        try:
            videos = self._call_api(
                f'v2/channels/{channel}/videos', v7_id,
                note='Resolving Kick UUIDv7 via channel listing')
        except Exception:
            return None
        ...
        for v in videos:
            v4_uuid = traverse_obj(v, ('video', 'uuid'))
            created_at = traverse_obj(v, 'created_at')
            if not v4_uuid or not created_at:
                continue
            created_sec = unified_timestamp(created_at.replace(' ', 'T') + 'Z')
            if created_sec is None:
                continue
            diff = abs(int(created_sec * 1000) - ts_ms)
            if diff < min_diff:
                min_diff = diff
                best = v4_uuid

        if best and min_diff <= 60_000:
            self.to_screen(f'[kick:vod] UUIDv7 {v7_id} → UUIDv4 {best} (Δ{min_diff}ms)')
            return best
        return None
    ...
    def _real_extract(self, url):
        ...
        # UUIDv7 detection: version nibble is at position 14 of the hyphenated UUID
        if video_id[14] == '7':
            channel = self._match_valid_url(url).group('channel')
            resolved = self._resolve_uuid_v7(channel, video_id)
            if resolved:
                video_id = resolved
            else:
                self.report_warning(
                    f'Could not resolve Kick UUIDv7 {video_id} to UUIDv4 — '
                    'video may be outside the last 30 VODs or the channel listing is unavailable.')
